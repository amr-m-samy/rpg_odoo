...
export class AttributeScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.lastRawAttributes = lodash.cloneDeep(
      this.player.attributes.rawAttributes,
    );
    this.attributesUiArray = [];
    this.panelComponent = new PanelComponent(
      this,
      assignPanelConfig["AttributeScene"],
    );
    this.panelComponent.setTitleText("Attributes");
    this.attributesBackground = this.panelComponent.panelBackground;
    const baseX =
      this.cameras.main.width / 2 - this.attributesBackground.width / 2;
    const baseY =
      this.cameras.main.height / 2 - this.attributesBackground.height / 2;
  ...
  createAttributesButtons() {
    ...
    this.attributesConfiguration.forEach((attribute, i) => {
      ...
      const yPosition =
        this.attributesBackground.y + startPosition + 25 * (i + 1);
      const xPosition = this.attributesBackground.x + 50;
      let minus_button = new ButtonMinus(
        this,
        xPosition,
        yPosition,
        "removeAttribute",
      ...
      this.interfaceController.interfaceElements[1][i].push(
        minus_button_interface,
      );
      this.interfaceController.interfaceElements[1][i].push(
        plus_button_interface,
      );
    });
    this.availableAttributesText = this.add
      .text(
        this.attributesBackground.x + 43,
        this.attributesBackground.y +
          startPosition +
          26 * (this.attributesConfiguration.length + 1),
  ...
  createAttributesInfo() {
    ...
    this.defenseText = this.add
      .text(
        ...
        `Defense: ${this.player.attributes.defense}`,
      )
      .setOrigin(0, 0.5);
    ...
    this.fleeText = this.add
      .text(
        ...
        `Flee: ${this.player.attributes.flee}`,
      )
      .setOrigin(0, 0.5);
    this.hitText = this.add
      .text(
        baseX,
        this.fleeText.y + this.fleeText.height + 5,
        `Hit: ${this.player.attributes.hit}`,
      )
      .setOrigin(0, 0.5);
  }
  ...
  addAttribute(payload) {
    this.player.attributesManager.addAttribute(
      payload.attribute,
      1,
      this.lastRawAttributes,
    );
  }

  removeAttribute(payload) {
    console.log(this.lastRawAttributes);
    this.player.attributesManager.removeAttribute(
      payload.attribute,
      1,
      this.lastRawAttributes,
    );
  ...
  resizeAll(size) {
    ...
    if (this.maxHealthText)
      this.maxHealthText.setPosition(
        baseX,
        this.defenseText.y + this.defenseText.height + 5,
      );
    if (this.criticalText)
      this.criticalText.setPosition(
        baseX,
        this.maxHealthText.y + this.maxHealthText.height + 5,
      );
    if (this.fleeText)
      this.fleeText.setPosition(
        baseX,
        this.criticalText.y + this.criticalText.height + 5,
      );
    if (this.hitText)
      this.hitText.setPosition(
        baseX,
        this.fleeText.y + this.fleeText.height + 5,
      );
  }
  ...
  setAttributesText() {
    if (this.scene.isActive()) {
      ...
      this.maxHealthText.setText(
        `Max Health: ${this.player.attributes.baseHealth}`,
      );
      this.criticalText.setText(
        `Critical: ${this.player.attributes.critical} %`,
      );
  ...
  update() {
    ...
    if (this.player && this.availableAttributesText)
      this.availableAttributesText.setText(
        "Available: " + this.player.attributes.availableStatPoints,
      );
  }
}
